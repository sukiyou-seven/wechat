<template>
	<u-picker ref="picker" :show="show" :closeOnClickOverlay="closeOnClickOverlay" :columns="columns" :title="title"
		...
		:confirmColor="confirmColor" :sukiyou_toolbar="sukiyou_toolbar" :sukiyou_month="sukiyou_month" @close="close"
		...
		methods: {
			...
			change(e) {
				...
				if (this.mode === 'time') {
					...
					let maxDate = ''
					try {
						maxDate = dayjs(`${year}-${month}`).daysInMonth()
					} catch (e) {
						//TODO handle the exception
						maxDate = dayjs(`${year}`).daysInMonth()
					}

					// year-month模式下，date不会出现在列中，设置为1，为了符合后边需要减1的需求
					if (this.mode === 'year-month') {
						date = 1
						// 若 sukiyou_month 为 false month列需要被隐藏
						if (!this.sukiyou_month) {
							const month = 1
						}
					}
			...
			updateIndexs(value) {
				...
				if (this.mode === 'time') {
					...
					values = [
						formatter('year', `${dayjs(value).year()}` + "年"),
						// 月份补0
						formatter('month', padZero(dayjs(value).month() + 1) + "月")
					]
			...
			getOriginColumns() {
				...
				const results = this.getRanges().map(({
					...
					let values = times(range[1] - range[0] + 1, (index) => {
						...
						value = type === 'year' ? `${value}` + "年" : uni.$u.padZero(value) + "月"
