<template>
	<view class="mrsong-charts" v-if="showChart">
		<view class="chart-title-box" :style="'text-align:'+align">
			<text class="chart-title">{{title}}</text>
		</view>
		<view class="charts-box">
			<view class="chart-unit" v-if="unit">
				单位：{{unit?`(${unit})`:''}}
			</view>
			<qiun-data-charts :opts="opts" type="column" :chartData="currentData" background="none" :animation="false"
				:ontouch="true" />
		</view>
	</view>
</template>
					...
					if (val) {
						...
						if (this.config) {
							let xAxis = opts.xAxis,
								yAxis = opts.yAxis.length ? opts.yAxis[0] : {},
